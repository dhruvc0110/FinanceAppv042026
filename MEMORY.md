...
- **Built: F9 Custom date-range reports** — Reports already had a `date` dimension + `between` op; F9 adds a first-class **Date range** From/To control to the Reports toolbar (`rptSetDateRange`/`rptClearDateRange`/`_rptSyncDateInputs`) that manages a single `{dimensionId:'date',op:'between'}` filter in `_rptConfig` — flows through `_rptBuildQuery` and saves/loads with the report. One-sided ranges use sentinel bounds (0000-01-01 / 9999-12-31). Verified ($2100 all → $700 for Mar–Apr).
- **Built: F24 Bulk edit** — checkbox column + select-all on the Transactions list (`_txBulkSel` Set, `_txPageIds`); dark bulk bar appears when ≥1 selected. Actions: **re-categorize** (set Debit or Credit account on all selected, clears the matching subId) and **delete selected** — both confirmed + wrapped in BEGIN/COMMIT/ROLLBACK; selection spans pages. Tagging was NOT included (no tag system — that's the Nice-to-have F10). **Limitation:** bulk re-categorize doesn't guard against setting dr==cr; power-user action. Verified (re-categorize 2 rows → Groceries, delete works).
- **Built: F20 Performance vs benchmark** — MCQ: "since-inception return comparison" (the app has NO historical price series — `MarketPrice` is current-price-only — so a time-series benchmark chart isn't feasible). Card on the US Equity summary: portfolio simple return = totalUnreal/totalCostBasis from open lots + current prices; user enters a benchmark name + return %; shows over/under-performance in pp. Benchmark assumption persisted in **localStorage** (`f20BenchPct`/`f20BenchName`), no DB schema change. `f20SetBench` re-renders the equity page. Verified (+20% vs 12% → +8.0 pp ahead).
- **Built: F29 Customizable dashboard** — hide/show ALREADY existed (Settings → Dashboard Tiles, `User.preferences.dashboardTiles`). F29 adds **reordering**: `User.preferences.dashboardTileOrder` (array of tile keys); `_dashOrderTiles` sorts the dashboard tiles by it (`_dashTileOpenKey`/detail unaffected — only tilesHtml ordered); Settings tile list now ordered with ▲▼ buttons (`dashMoveTile`). Verified (move Total Assets above Net Worth → dashboard reflects). Note: 'Pending' tile isn't in the Settings ALL_TILES list (pre-existing) so it sorts to the end.
- **Built: F12 Category rollover + F14 Overspend coverage** (MCQ: lightweight overlay, NOT a full envelope rebuild — the budget is a flat planning grid). New **"Actual vs Budget"** view toggle on the Budget page (`_bgtView` plan|actual, `_renderBudgetActual`, month selector `_bgtAvbMonth`). Per expense category for the chosen month: Budget, Actual (dr−cr from txns), Remaining (=budget−actual), **Rollover-in** (cumulative prior budget−actual, derived — F12), **Available** (=cumBudget−cumActual). **F14 Move budget**: `bgtMoveBudget` shifts a budgeted amount between two categories for the month (`_bgtAdjustBudget`, BEGIN/COMMIT). Rollover is COMPUTED, not a stored envelope balance. Verified (May unspent $100 → Jun overspend covered → available $0; move $200 Dining→Groceries → covered).
### Decided: Floating pill (Dictate/New/Reports) is draggable.
