...
### Decided: Multi-line Journal Entries — True header+lines ledger model. (Phase 1 shipped, unpushed.)
- **Why:** User needs balanced entries where the count of DR and CR lines is free (not the 1-DR↔1-CR shape the `"Transaction"` table enforces). User explicitly chose the **True header+lines** model over alternatives and asked for a recovery point first (git tag `stable-pre-je` → commit 0552eb6, pushed).
- **Model:** Two new tables. `JournalEntry` (id, trxDate, period, description, notes, importSource='Journal', createdAt, updatedAt) is the header. `JournalLine` (id, jeId, side 'DR'/'CR', accountId, subId, amount **positive**, lineNo, createdAt) — one side per line. Entry must balance (ΣDR == ΣCR within 0.005) but line counts per side are free. Tables added to BOTH `_initCoreSchema` (fresh DBs) and the `startApp` migration region as `CREATE TABLE IF NOT EXISTS` (existing DBs). No ALTER needed; Drive sync is whole-DB.
- **Balance-engine integration (the key low-blast-radius move):** ONE pass added to `buildBalanceMap()` — a `JournalLine JOIN JournalEntry` query grouped by (accountId, side), same period filter (on `je.period`), folding DR→drMap / CR→crMap. This makes Dashboard / Financial Statements / Budget / Account balances all reflect JEs with no other wiring. Empty JE tables → JOIN returns nothing → existing balances unchanged.
- **UI:** New **"Journal Entries"** sub-tab in the Transactions composite (`_TX_COMBO_TABS`, dispatch in `_renderTxComboContent`). `renderJournalEntries()` = list (date / desc / line count / amount / Balanced badge / View+Delete) + `#jeModal`. Modal: header date+desc+notes, dynamic line rows (side select, account+sub picker keyed `jeAcct_L{n}`/`jeSub_L{n}`, amount), `+ Add line`, live `jeRecalc()` showing Debits/Credits/Balanced, **Save disabled until balanced & ΣDR>0**. Handlers: `jeOpenModal` (seeds 1 DR + 1 CR), `jeAddLine`/`jeRemoveLine`, `jeSaveEntry` (re-validates: ≥2 lines, ≥1 DR, ≥1 CR, balanced, every filled line needs account+amount>0; skips fully-empty lines), `jeViewEntry` (alert summary), `jeDeleteEntry` (confirm → deletes lines then header).
- **Verified end-to-end** in a throwaway in-memory sql.js DB (login-gated app, so no Drive DB): schema creates, modal opens with 2 lines + save disabled, live balance gate flips at balance, 3-line split saves with correct lineNo, `buildBalanceMap` folds lines correctly and respects period filter (other periods → 0), delete cascades header+lines and balances return to 0. Visual screenshot not possible (auth overlay covers injected DOM).
- **Rejected:** (a) Reusing `"Transaction"` with a shared group-id (would need every balance/list/report query to re-aggregate by group — high blast radius). (b) Storing signed amounts on one column instead of an explicit `side` (loses the DR/CR semantics the rest of the app reads).
- **NOT done / Phase 2 follow-ups:** Account drill-down transaction lists do **not** yet itemise JE lines (balances include them, but clicking an account won't show the JE line). Editing an existing JE (modal is create-only; delete + recreate for now). JE lines in Reports pivot. CSV export of JEs. Inline JE line reordering.
